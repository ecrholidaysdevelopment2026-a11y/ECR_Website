...
const MapPicker = ({
  ...
  useEffect(() => {
    ...
    const normalized = data.map(normalizePosition).filter(Boolean);
  ...
  const emitChange = (list) => {
    ...
    const links = list.map(
      (p) => `https://www.openstreetmap.org/?mlat=${p.lat}&mlon=${p.lng}`,
    );
  ...
  const ClickHandler = () => {
    useMapEvents({
      click(e) {
        setMarkers((prev) => {
          const updated = multiple ? [...prev, e.latlng] : [e.latlng];
          emitChange(updated);
          return updated;
        });
      },
    ...
    <>
      ...
      <MapContainer center={[20, 78]} zoom={5} className="h-full w-full">
        ...
        {markers?.map((pos, index) => (
          <Marker key={index} position={[pos.lat, pos.lng]} icon={homeDivIcon}>
            <Popup
              closeButton={false}
              ...
              <div className="w-[241px] rounded-xl overflow-hidden">
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between text-sm font-semibold">
                    <span className="truncate max-w-[161px]">{pos.title}</span>
                    {pos.rating && (
