...
export default function BannerSection({ initialData = null }) {
  ...
  const selectedLocation = locations?.find((item) => item.slug === destination);
  ...
  return (
    <div className="flex flex-col items-center text-center w-full px-4">
      ...
      <div className="bg-white shadow-xl rounded-full p-3 hidden md:flex items-center gap-3">
        <div
          className="flex items-center gap-3 border-r pr-5 relative"
          ref={destinationRef}
        >
          ...
          >
            ...
            <p
              className={`text-sm ${destination ? "text-black " : "text-gray-400"}`}
            >
              {selectedLocation
                ? selectedLocation.locationName
                : "Select destination"}
          ...
          {showDestination && (
            <div className="absolute top-15 left-0 w-72 bg-white rounded-xl shadow-xl border border-gray-300 z-50 overflow-y-scroll  h-auto max-h-[260px] scrollbar-hide">
              {locations?.map((item) => (
                <button
                  key={item._id}
                  onClick={() => {
  ...
  "
        ...
        <div className="flex items-center gap-3 px-5 relative" ref={guestRef}>
          ...
          >
            ...
            <p className="text-sm">
              {totalGuests} Guest{totalGuests !== 1 ? "s" : ""}, {rooms} Room
              {rooms !== 1 ? "s" : ""}
            </p>
          </div>
          {showGuestDropdown && (
